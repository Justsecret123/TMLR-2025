...
def load_dataset():
    ...
    # Load the data
    dataset = tf.data.TFRecordDataset("./tfrecords/STELLA/mg_val_binary_full_completed.tfrecord")
    # Create a graph spec
...
def _get_dataset(split, *, shuffle=False, filter_fn=None,
  ...
  if split=="train":
    filenames = ["./tfrecords/STELLA/mg_train_binary_full_completed.tfrecord"]
  elif split=="val":
    filenames=["./tfrecords/STELLA/mg_val_binary_full_completed.tfrecord"]
  else:
    filenames=["./tfrecords/STELLA/mg_val_binary_full_completed.tfrecord"]
...
def save_results(history):
    ...
    with open("./results/STELLA/results_GCN_binary.txt", "a+") as file:
...
def save_scores(scores):
    ...
    with open("./results/STELLA/scores_GCN_binary.txt", "a+", encoding="utf-8") as file:
        # Text to write 
        file.write(f"{first_line}\n")
