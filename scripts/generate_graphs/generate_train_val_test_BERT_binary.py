...
import generate_graphs_BERT as generate_graphs
import pickle
...
with open("nx_dataset_objectification_train_2_V3.pkl","rb") as file: 
    ...
    with tf.io.TFRecordWriter("mg_training_2_V3_completed.tfrecord") as writer: 
...
# Read the binary pickle file
with open("nx_dataset_objectification_val_2_V3.pkl","rb") as file: 
    # Load dataset and store into the variable
    nx_dataset = pickle.load(file)
    # Get the val len 
    ...
    i = 0
    # Generate the training set
    with tf.io.TFRecordWriter("mg_validation_2_V3_completed.tfrecord") as writer: 
        # Loop through the graphs
        for id, graph in nx_dataset.items():
            # Get the node sets dict
            node_sets = generate_graphs.get_node_sets_dict(graph, id)
