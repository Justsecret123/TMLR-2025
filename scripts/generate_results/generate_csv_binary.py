...
def get_data_dict(results): 
    ...
    for i in range(0,len(results), 7):
        ...
        training_auc.append(convert_to_float(results_set[3]))
        val_auc.append(convert_to_float(results_set[4]))
        training_loss.append(convert_to_float(results_set[5]))
        val_loss.append(convert_to_float(results_set[6]))
...
def main(results): 
    ...
    dataset = pd.DataFrame(columns=["Character", "Relationship", "Emotion", "Attribute", 
                                "Place", "Context", "Speech", "Training acc", "Val acc",
                                "Training auc", "Val auc", "Training loss", "Val loss"], data=data)
    # Save the dataset 
